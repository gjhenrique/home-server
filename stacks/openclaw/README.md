...
- **Control UI `proxy_attribution_required`**: OpenClaw 2026.8.1 treats every IP in `gateway.trustedProxies` as a proxy hop, including the client address in `X-Forwarded-For`. Do **not** use `192.168.0.0/16` (LAN clients then fail). Trust only the Docker bridge gateway OpenClaw actually sees, e.g. `["192.168.144.1"]`. Caddy labels overwrite `X-Forwarded-For` with the real client. Recreate the gateway after label changes so caddy-docker-proxy reloads.
